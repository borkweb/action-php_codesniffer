...
if [ "${INPUT_ONLY_SNIFF_DIFF}" = true ]; then
	php /usr/local/bin/phpcs.phar --standard=${INPUT_STANDARD} --extensions=php --report=checkstyle -q $(git diff --name-only --diff-filter=ACMRTUXB origin/${GITHUB_BASE_REF} | grep -E \.php$) \
